...
# --- CSS ORIGINALE (CON FIX DIMENSIONI) ---
st.markdown("""
    <style>
    ...
    /* 2026 UNLOCKED - AGGIUSTATO */
    .unlocked-title {
        color: white; 
        font-family: 'Orbitron', sans-serif; 
        font-size: clamp(40px, 12vw, 110px); /* Ridotto il max per evitare che sfalsi */
...
def show_party_visuals():
    ...
    class FW {
        constructor(){
            ...
            this.sy=Math.random()*-4-8; // SPINTA VERSO L'ALTO
            this.sx=Math.random()*2-1; this.ex=false;
        }
        up(){
            x.fillStyle=this.color; x.beginPath(); x.arc(this.x,this.y,3,0,7); x.fill();
            this.x+=this.sx; this.y+=this.sy; this.sy+=0.12; // GRAVITA'
            if(this.sy>=-0.5){ // ESPLOSIONE AL CULMINE
                this.ex=true;
                for(let i=0;i<50;i++){
    ...
    function anim(){
        ...
        fws=fws.filter(f=>{f.up(); return !f.ex;});
        ps=ps.filter(p=>{
            x.globalAlpha=p.l; x.fillStyle=p.c; x.beginPath(); x.arc(p.x,p.y,2,0,7); x.fill();
            p.x+=p.sx; p.y+=p.sy; p.sy+=0.05; p.l-=0.02; return p.l>0;
        });
