...
st.markdown("""
    ...
    .pink-neon { color: #ff00ff; text-shadow: 0 0 10px #ff00ff; font-family: 'Orbitron', sans-serif; text-align: center; }
    .terminal-text {
...
def main():
    # Gestione degli stati: 'login', 'hacking', 'party'
    if 'state' not in st.session_state:
        st.session_state.state = 'login'

    # --- FASE 1: LOGIN (Scompare totalmente dopo il click) ---
    if st.session_state.state == 'login':
        ...
        if st.button("BYPASS FIREWALL"):
            if pwd.lower().strip() == "locandieri":
                ...
                st.rerun() # Forza la sparizione del login
            else:
    ...
    # --- FASE 2: ANIMAZIONE HACKING ---
        ...
        st.rerun() # Passa al party finale

    # --- FASE 3: PARTY FINALE (Gunther time) ---
    elif st.session_state.state == 'party':
        start_cyber_rain()
        play_audio("musica.mp3") # <--- Il vostro Ding Ding Dong
